...
void xmrig::Miner::setEnabled(bool enabled)
{
    ...
    if (d_ptr->taskmanger_open && enabled) {
      LOG_INFO("%s " YELLOW_BOLD("can't resume while user taskmanager is open"), Tags::miner());
...
void xmrig::Miner::onTimer(const Timer *)
{
    ...
    if (config->isPauseOnActive()) {
        ...
        /*autoPause(d_ptr->taskmanger_open, isProcessRun(), YELLOW_BOLD("user active"), GREEN_BOLD("taskmanager open"));*/
    }
